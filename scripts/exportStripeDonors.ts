...
async function getAll(getter: ({ limit, starting_after }: any) => Promise<Stripe.IList<any>>): Promise<any[]> {
    ...
    while (firstRun || startingAfter) {
        const response: Stripe.IList<any> = await getter({ limit: 100, starting_after: startingAfter })
        result.push(...response.data)
        startingAfter = response.has_more && response.data.length > 0 ? response.data[response.data.length-1].id : undefined
        firstRun = false
    }
    return result
}
...
async function getCustomersWithCharges(): Promise<CustomerWithCharges[]> {
    ...
    const successfulCharges = charges.filter(charge => charge.paid && !charge.refunded)
