...
const DATA_FILE = "/Users/mispy/Bulk imports/2018 V-Dem Dataset/Original/Country_Year_V-Dem_Extended_CSV_v8/V-Dem-CY+Others-v8.csv"
...
async function importData() {
    ...
    const standardize = {"Ivory Coast":"Cote d\'Ivoire","Republic of the Congo":"Congo","German Democratic Republic":"East Germany","The Gambia":"Gambia","Burma/Myanmar":"Myanmar","Papal States":"Vatican","São Tomé and Príncipe":"Sao Tome and Principe","Timor-Leste":"Timor","United States of America":"United States","Democratic Republic of Vietnam":"Vietnam","Würtemberg":"Wurtemberg"} as {[key: string]: string|undefined}

    const entitiesUniq = ["Afghanistan","Angola","Albania","United Arab Emirates","Argentina","Armenia","Australia","Austria","Azerbaijan","Burundi","Baden","Belgium","Benin","Burkina Faso","Bangladesh","Bulgaria","Bahrain","Bosnia and Herzegovina","Belarus","Bolivia","Brazil","Barbados","Brunswick","Bhutan","Bavaria","Botswana","Central African Republic","Canada","Switzerland","Chile","China","Cote d\'Ivoire","Cameroon","Democratic Republic of Congo","Congo","Colombia","Comoros","Cape Verde","Costa Rica","Cuba","Cyprus","Czech Republic","East Germany","Germany","Djibouti","Denmark","Dominican Republic","Algeria","Ecuador","Egypt","Eritrea","Spain","Estonia","Ethiopia","Finland","Fiji","France","Gabon","United Kingdom","Georgia","Ghana","Guinea","Gambia","Guinea-Bissau","Equatorial Guinea","Greece","Guatemala","Guyana","Hesse-Darmstadt","Hong Kong","Hesse-Kassel","Honduras","Hamburg","Croatia","Haiti","Hungary","Hanover","Indonesia","India","Ireland","Iran","Iraq","Iceland","Israel","Italy","Jamaica","Jordan","Japan","Kazakhstan","Kenya","Kyrgyzstan","Cambodia","South Korea","Kuwait","Laos","Lebanon","Liberia","Libya","Sri Lanka","Lesotho","Lithuania","Luxembourg","Latvia","Morocco","Mecklenburg Schwerin","Moldova","Madagascar","Modena","Maldives","Mexico","Macedonia","Mali","Myanmar","Montenegro","Mongolia","Mozambique","Mauritania","Mauritius","Malawi","Malaysia","Namibia","Niger","Nigeria","Nicaragua","Netherlands","Norway","Nepal","Nassau","New Zealand","Oldenburg","Oman","Pakistan","Panama","Peru","Philippines","Papua New Guinea","Poland","Vatican","North Korea","Parma","Portugal","Paraguay","Palestine/British Mandate","Palestine/West Bank","Palestine/Gaza","Qatar","Romania","Russia","Rwanda","Saudi Arabia","Saxe-Weimar-Eisenach","Sudan","Senegal","Singapore","Solomon Islands","Sierra Leone","El Salvador","Somalia","Somaliland","Piedmont-Sardinia","Serbia","South Sudan","Sao Tome and Principe","Suriname","Slovakia","Slovenia","Sweden","Swaziland","Saxony","Seychelles","Syria","Chad","Togo","Thailand","Tajikistan","Turkmenistan","Timor","Tuscany","Trinidad and Tobago","Tunisia","Turkey","Taiwan","Two Sicilies","Tanzania","Uganda","Ukraine","Uruguay","United States","Uzbekistan","Republic of Vietnam","Venezuela","Vietnam","Vanuatu","Wurtemberg","Kosovo","Yemen","South Yemen","South Africa","Zambia","Zimbabwe","Zanzibar"]

    await db.transaction(async t => {
        await t.execute(`DELETE dv FROM data_values dv JOIN variables v ON dv.variableId=v.id JOIN datasets d ON v.datasetId=d.id WHERE d.namespace='vdem'`)
        ...
        const variableRows = await t.query(`SELECT v.code, v.id FROM variables v JOIN datasets d ON d.id=v.datasetId WHERE d.namespace='vdem'`)
        ...
        while (row !== undefined) {
            if (!variableIds.length) {
                ...
                for (let i = VARIABLE_START_COLUMN; i < row.length; i++) {
                    ...
                    valueRows.push([row[i], parseInt(year), entityId, variableId])
                }
            }
        ...
        await t.execute("delete v from variables v left join data_values dv on dv.variableId=v.id join datasets d on d.id=v.datasetId where d.namespace='vdem' and dv.id IS NULL;")
    })
}
